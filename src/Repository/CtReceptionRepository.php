...
//    }
    ...
    public function findNombreReceptionPayante($date, $type, $motif, $centre, $utilisation): ?int
    {
        return $this->createQueryBuilder('c')
            ...
            ->andWhere('c.rcp_created LIKE :val1')
            ->andWhere('c.rcp_is_active = :val2')
            ->andWhere('c.ct_type_reception_id IN (:val3)')
            ->andWhere('c.ct_motif_id = :val4')
    ...
    public function findNombreReceptionPayanteCalculable($date, $type, $motif, $centre, $utilisation, $min, $max): ?int
    {
        return $this->createQueryBuilder('c')
            ->select('COUNT(c) ')
            ->andWhere('c.rcp_created LIKE :val1')
            ...
            ->andWhere('c.ct_centre_id IN (:val5)')
            ->andWhere('c.ct_utilisation_id = :val6')
            ->andWhere('c.ct_vehicule_id.vhc_poids_total_charge >= :val7')
            ->andWhere('c.ct_vehicule_id.vhc_poids_total_charge < :val8')
            ->setParameter('val1', '%'.$date.'%')
            ->setParameter('val2', 1)
            ->setParameter('val3', $type)
            ->setParameter('val4', $motif)
            ->setParameter('val5', $centre)
            ->setParameter('val6', $utilisation)
            ->setParameter('val7', $min)
            ->setParameter('val8', $max)
            ->setMaxResults(1)
            ->getQuery()
            ->getSingleScalarResult()
            //->getOneOrNullResult()
